...
class ActualizarAsignaturaTest {
    @Autowired
    ...
    @Test
    void actualizarAsignaturaTest() throws URISyntaxException {
        final String baseUrl = "http://localhost:" + randomServerPort + "/asignatura/" + 1;
        URI uri = new URI(baseUrl);
        AsignaturaInputDTO asignatura = new AsignaturaInputDTO("1",
                "Sistemas", "requiere libro de texto", new Date(2003-11-23), new Date(2006-11-23));

        HttpHeaders headers = new HttpHeaders();
        HttpEntity<AsignaturaInputDTO> request = new HttpEntity<>(asignatura, headers);

        ResponseEntity<AsignaturaOutputDTO> result = this.testRestTemplate.exchange(
                uri, HttpMethod.PUT, request, AsignaturaOutputDTO.class);

        //Verify request succeed
        Assertions.assertEquals(200, result.getStatusCodeValue());
        Assertions.assertEquals("Sistemas", Objects.requireNonNull(result.getBody()).getAsignatura());
    }
}
